...
export default function EditProductpage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Upload thư viện ảnh bổ sung nếu có
      ...
      // 3. Gọi đúng 1 lệnh API duy nhất để CẬP NHẬT toàn bộ dữ liệu vào bảng products
      ...
      console.error(error);
      toast.error(error.message || 'Failed to update product.', { style: { background: '#e5534b', color: '#fff' } });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-3xl border border-[#30363d] rounded-xl bg-[#161b22] text-white">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-300">Price (VND) *</label>
            <input
              required
              type="text"
              name="price"
