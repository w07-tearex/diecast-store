...
export default function NewProductForm() {
  ...
  // Form State - Toàn bộ dữ liệu nằm cục bộ, KHÔNG GỌI API KHI ĐANG GÕ
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Gọi đúng 1 lệnh API duy nhất để lưu toàn bộ dữ liệu vào bảng products
      ...
      console.error(error);
      toast.error(error.message || 'Failed to save product.', { style: { background: '#e5534b', color: '#fff' } });
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2 col-span-2">
            <label className="text-sm font-medium text-gray-300">Tên sản phẩm *</label>
            <input
              required
              name="name"
              value={formData.name}
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-300">Hãng sản xuất</label>
            <select
              name="brand"
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-300">Giá (VND) *</label>
            <input
              required
              type="text"
              name="price"
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-300">Tồn kho (Stock) *</label>
            <input
              required
              type="number"
              min="0"
